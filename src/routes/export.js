...
router.get('/:competitionId/csv', auth, catchAsync(async (req, res, next) => {
  ...
  if (competition.organizerId && competition.organizerId.toString() !== req.organizer.id) {
    return next(new AppError('You do not have permission to export this competition', 403));
  }

  // Check if competition has final rankings
  if (!competition.finalRankings || competition.finalRankings.length === 0) {
    return next(new AppError('No rankings available. Competition may not be completed or has no participants.', 400));
  }
...
router.get('/:competitionId/json', auth, catchAsync(async (req, res, next) => {
  const { competitionId } = req.params;
...
router.get('/:competitionId/rounds/csv', auth, catchAsync(async (req, res, next) => {
  const { competitionId } = req.params;
  ...
  if (roundData.length === 0) {
    return next(new AppError('No round data available', 400));
  }
